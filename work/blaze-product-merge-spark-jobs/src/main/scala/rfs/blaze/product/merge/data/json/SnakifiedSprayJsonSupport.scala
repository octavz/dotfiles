...
trait SnakifiedSprayJsonSupport extends DefaultJsonProtocol {
  ...
  override protected def fromField[T](value: JsValue, fieldName: String)(implicit reader: JsonReader[T]): T = {
    try super.fromField(value, fieldName)(reader) catch {
      case e: DeserializationException ⇒ deserializationError(s"Failed to read $fieldName", e)
    }
  }
